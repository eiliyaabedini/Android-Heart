...
/**
 * @return An Observable of T that emits an item of type T
 * when the original Observable emits an [Optional] that contains a value
 */
fun <T : Any> Observable<Optional<T>>.filterIsSome(): Observable<T> {
    return filter { it.isSome }.map { it.get() }
}
